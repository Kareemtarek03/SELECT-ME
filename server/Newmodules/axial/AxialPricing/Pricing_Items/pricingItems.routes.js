...
router.get("/categories", authenticateToken, isAdmin, PricingItemsController.getCategories);
router.get("/categories/name/:name", authenticateToken, isAdmin, PricingItemsController.getCategoryByName);
router.get("/categories/:id", authenticateToken, isAdmin, PricingItemsController.getCategoryById);
router.post("/categories", authenticateToken, isAdmin, PricingItemsController.createCategory);
router.patch("/categories/:id", authenticateToken, isAdmin, PricingItemsController.updateCategory);
router.delete("/categories/:id", authenticateToken, isAdmin, PricingItemsController.deleteCategory);
...
router.delete("/items/:id", authenticateToken, isAdmin, PricingItemsController.deleteItem);
...
router.post("/test-recalc", async (req, res) => {
    ...
    try {
        const { recalculateAllAccessoriesPrices } = await import("../shared/pricingUtils.js");
        const { PrismaClient } = await import("@prisma/client");
        const prisma = new PrismaClient();

        // Get Bolts & Nuts price
        const boltsItem = await prisma.pricingItem.findFirst({ where: { sr: 21 } });
        ...
        const results = await recalculateAllAccessoriesPrices(boltsItem.priceWithVat);
