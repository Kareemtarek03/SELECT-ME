...
export const DOLLAR_RATE_SR = 22;

export const ELECTRICAL_MOTORS_SR = 20;

export const ELECTRICAL_CABLES_SR = 18; // Electrical Cables (Sweedy Factor)
 ...
 * Get the Dollar Rate (sr=22) price with VAT from pricing_items table
 ...
 * Get the Electrical Motors (sr=20) price with VAT from pricing_items table
 ...
 * Get the Electrical Cables (Sweedy Factor) (sr=18) price with VAT
 ...
 * Get the Electrical Component Factor (sr=19) price with VAT
 ...
 * @param {number} dollarRate - Dollar Rate (sr=22) price with VAT

 * @param {number} electricalMotorsFactor - Electrical Motors (sr=20) price with VAT
 ...
 * Formula: Price w/o VAT Per Meter (Cable) × Electrical Cables Factor (sr=18) × VAT_RATE
 ...
 * Formula: U.P Price w/o VAT (Cable Lugs) × Electrical Component Factor (sr=19) × VAT_RATE
 ...
 * Formula: U.P Price w/o VAT (Cable Heat Shrink) × Electrical Component Factor (sr=19) × VAT_RATE
 ...
 * Formula: U.P Price w/o VAT (Flexible Connector) × Electrical Component Factor (sr=19) × VAT_RATE
 ...
 * Formula: U.P Price w/o VAT (Gland) × Electrical Component Factor (sr=19) × VAT_RATE
 ...
 * Formula: U.P Price w/o VAT (Brass Bar) × Electrical Cables Factor (sr=18) × VAT_RATE
 ...
 * Formula: U.P Price w/o VAT (Electrical Box) × Electrical Component Factor (sr=19) × VAT_RATE
 ...
 * Compute Current (A) with Safety Factor (25%)

 * Excel column AN: =1.25 * F3 (Rated current)
...
export const computeCableSize = (currentWithSF) => {
  ...
  if (I < 21) return "3x1.5 RM";

  if (I < 27) return "3x2.5 RM";

  if (I < 36) return "3x4 RM";

  if (I < 43) return "3x6 RM";

  if (I < 57) return "3x10 RM";

  if (I < 71) return "3x16 RM";

  if (I < 94) return "3x25 RM";

  if (I < 114) return "3x35 RM";

  if (I < 136) return "3x50 SM";

  if (I < 169) return "3x70 SM";

  if (I < 205) return "3x95 SM";

  if (I < 234) return "3x120 SM";
 ...
 * Compute Flexible Connector Size based on Power (kW)

 * Excel column AX: =IF(C3<=15,"3/4\"",IF(C3<=37,"1\"",IF(C3<=110,"1.5\"","N/A")))
...
const GLAND_BASE_PRICES = { '3/4"': 85, '1"': 110, '1.5"': 130 };
 ...
 * Compute Electrical Box Size based on Flexible Connector Size

 * Excel column BG: =IF(AX3="3/4\"","110 x 110 x 55",IF(AX3="1\"","160 x 140 x 55",...))
 ...
 * Compute all derived fields from motor's base data (rated current, power kW).

 * Returns an object with auto-computed fields that can be merged into motor data.

 * Only populates fields that can be derived; leaves others as-is.
...
export const computeDerivedFields = (motorData) => {
  ...
  const cableCurrent = computeCableCurrent(ratedCurrent);
...
export const stripCalculatedFields = (data) => {
  ...
  // Remove JSON format w/o VAT fields (not in Excel, computed via lookup tables)

  delete stripped["Price w/o VAT Per Meter (Cable)"];

  delete stripped["U.P Price w/o VAT (Cable Lugs)"];

  delete stripped["U.P Price w/o VAT (Cable Heat Shrink)"];

  delete stripped["U.P Price w/o VAT (Flexible Connector)"];

  delete stripped["U.P Price w/o VAT (Gland)"];

  delete stripped["U.P Price w/o VAT (Brass Bar)"];
 ...
 * Used when Dollar Rate (sr=22) or Electrical Motors (sr=20) pricing changes
 ...
 * Used when Electrical Cables (Sweedy Factor) (sr=18) pricing changes
 ...
 * Used when Electrical Component Factor (sr=19) pricing changes
 ...
 * Used when sr=18 or sr=19 pricing changes - this handles all the new calculated fields
