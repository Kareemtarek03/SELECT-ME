...
export default function MotorPage() {
  ...
  const handleDelete = async (motorId) => {
    try {
      ...
      const resp = await fetch(
        `${API_BASE}/api/motor-data/${motorId}`,
        { method: "DELETE" },
      );
      if (!resp.ok) {
  ...
  const handleSave = async () => {
    try {
      ...
      const url = isEdit
        ? `${API_BASE}/api/motor-data/${editingMotor.id}`
        : `${API_BASE}/api/motor-data/`;
  ...
  const columns = React.useMemo(() => {
    ...
    motors.forEach((m) =>
      Object.keys(m || {}).forEach((k) => existingKeys.add(k)),
    );

    return COLUMN_ORDER.filter((col) => existingKeys.has(col)).map((col) => ({
      key: col,
      label: col,
    }));
  }, [motors]);
  ...
  return (
    <Box p={4} bg="var(--bg-page)" color="var(--text-primary)">
      ...
      <Stack direction={"row"} justify="space-between" flexWrap="wrap" gap={4}>
            ...
            <input
              ...
              onChange={async (e) => {
                ...
                try {
                  ...
                  const resp = await fetch(
                    `${API_BASE}/api/motor-data/upload`,
                    {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify({
            ...
            <Button
              ...
              onClick={async () => {
                try {
                  ...
                  const resp = await fetch(
                    `${API_BASE}/api/motor-data/export`,
                  );
                  if (!resp.ok) {
      ...
      <Dialog.Root open={openDialog}>
        <Portal>
          ...
          <Dialog.Positioner>
            ...
            >
              <Dialog.Header>
                <Dialog.Title color="var(--text-primary)">Delete Motor</Dialog.Title>
              </Dialog.Header>
              <Dialog.Body>
                {selectedMotor ? (
                      ...
                      return (
                        <Box key={col}>
                          <Text
                            fontSize="sm"
                            mb={1}
                            color={isCalculated ? "var(--success)" : "var(--text-muted)"}
                          >
                            {col}
                            {isCalculated && (
                              <Text
                          ...
                          {isCalculated ? (
                            ...
                            <Input
                              ...
                              value={formData[col] ?? ""}
                              onChange={(e) =>
                                handleFormChange(col, e.target.value)
                              }
                              placeholder={col}
                            />
                          )}
